...
func TestIsPublic(t *testing.T) {
	testnames := []struct {
		TestName string
		Annot    *typast.Annot
		CheckFn  func(*typast.Annot) bool
		Expected bool
	}{
		{
			TestName: "IsPublic: function name start with lower case",
			Annot:    &typast.Annot{Decl: &typast.Decl{Type: &typast.FuncDecl{Name: "someFunc"}}},
		...
		},
		{
			...
			Annot:    &typast.Annot{Decl: &typast.Decl{Type: &typast.InterfaceDecl{}}},
			CheckFn:  typast.IsFunc,
			Expected: false,
		},
		{
		...
		},
		{
			...
			Annot:    &typast.Annot{Decl: &typast.Decl{Type: &typast.StructDecl{}}},
	...
	for _, tt := range testnames {
		t.Run(tt.TestName, func(t *testing.T) {
			require.Equal(t, tt.Expected, tt.CheckFn(tt.Annot))
		})
	}
}
